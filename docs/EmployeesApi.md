...
with MergeHRISClient.ApiClient(configuration) as api_client:
    ...
    # and optional values
    try:
        api_response = api_instance.employees_create(x_account_token, run_async=run_async, employee_request=employee_request)
        pprint(api_response)
    except MergeHRISClient.ApiException as e:
        print("Exception when calling EmployeesApi->employees_create: %s\n" % e)
```
...
with MergeHRISClient.ApiClient(configuration) as api_client:
    ...
    include_sensitive_fields = True # bool | Whether to include sensetive fields (such as social security numbers) in the response. (optional)
