...
from MergeHRISClient.api import departments_api
...
with MergeHRISClient.ApiClient(configuration) as api_client:
    ...
    # and optional values
    try:
        api_response = api_instance.departments_list(x_account_token, created_after=created_after, created_before=created_before, cursor=cursor, modified_after=modified_after, modified_before=modified_before, page_size=page_size, remote_id=remote_id)
        pprint(api_response)
    except MergeHRISClient.ApiException as e:
        print("Exception when calling DepartmentsApi->departments_list: %s\n" % e)
```
...
> Department departments_partial_update(x_account_token, id)
...
with MergeHRISClient.ApiClient(configuration) as api_client:
    ...
    patched_department = PatchedDepartment(
        id="5b3c1341-a20f-4e51-b72c-f3830a16c97b",
        remote_id="23456",
        name="Engineering",
    ) # PatchedDepartment |  (optional)

    # example passing only required values which don't have defaults set
    try:
        api_response = api_instance.departments_partial_update(x_account_token, id)
        pprint(api_response)
    except MergeHRISClient.ApiException as e:
    ...
    # and optional values
    try:
        api_response = api_instance.departments_partial_update(x_account_token, id, run_async=run_async, patched_department=patched_department)
        pprint(api_response)
    except MergeHRISClient.ApiException as e:
        print("Exception when calling DepartmentsApi->departments_partial_update: %s\n" % e)
```
